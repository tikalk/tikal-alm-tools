...
# $6 - temp.root.folder - temporary work folder root
...
if [ "x$1" = "x" ] || [ "x$2" = "x" ] || [ "x$3" = "x" ] || [ "x$4" = "x" ] || [ "x$5" = "x" ] || [ "x$6" = "x" ]; then
  ...
  echo "where:"
	...
	echo " temp.root.folder - temporary work folder root"
  exit -1
fi

ant -buildfile cvs2svn2git.xml -Dcvs2svn.tool.folder=$cvs2svn_tool_folder -Dcvs.repository.root=$cvs_repository_root -Drepository.name=$repository_name -Dauthors.file.path=$authors_file_path -Dgit.remote.uri=$git_remote_uri -Dtemp.root.folder=$temp_root_folder
